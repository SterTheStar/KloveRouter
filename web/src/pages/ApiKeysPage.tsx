...
export default function ApiKeysPage() {
  ...
  const create = async () => {
    ...
    try {
      ...
      success(
        "API key generated",
        "You can reveal or copy it later from the API keys page.",
      );
    } catch (e: any) {
      setError(e.message);
    ...
    <div className="w-full space-y-6 p-6">
      ...
      {keys.length === 0 ? (
        <div className="rounded-xl border border-dashed p-12 text-center">
          ...
          <h2 className="mt-3 font-heading text-lg font-medium">No API keys</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Generate a key to use the Klove proxy.
          </p>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Copy it now or manage this key later from the API keys page.
            </DialogDescription>
          </DialogHeader>
          {secret && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  size="icon"
                  onClick={() => copy(secret.raw_key)}
                  title="Copy API key"
                >
                  <Copy className="size-4" />
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">{secret.warning}</p>
            </div>
          )}
          <DialogFooter>
      ...
      <ConfirmDialog
        ...
        message={`Revoke ${removeTarget?.name}?`}
        confirmLabel="Revoke"
        onConfirm={remove}
