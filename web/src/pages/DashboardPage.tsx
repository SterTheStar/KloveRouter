...
export default function DashboardPage({ onNavigate }: { onNavigate: (page: "provider-detail", providerId: string) => void }) {
  ...
  useEffect(() => { load(); }, [load]);
  const remove = async () => { if (!target) return; setDeleting(true); try { await providers.remove(target.id); setList((items) => items.filter((item) => item.id !== target.id)); setTarget(null); } catch (e: any) { setError(e.message); } finally { setDeleting(false); } };
