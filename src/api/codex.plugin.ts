...
export const antigravityPublicPlugin = (app: Elysia) =>
  app.get("/antigravity/callback", async ({ query, set }) => {
    try {
      if (!query.code || !query.state) { set.status = 400; return "Missing OAuth code or state"; }
      await antigravityAuthService.completeLogin(query.code, query.state);
      return new Response(ANTIGRAVITY_CALLBACK_HTML, { headers: { "Content-Type": "text/html" } });
    } catch (error: any) { set.status = 400; return `Antigravity OAuth failed: ${error.message}`; }
  }, { query: t.Object({ code: t.Optional(t.String()), state: t.Optional(t.String()), error: t.Optional(t.String()) }) });
...
export const antigravityUsagePlugin = (app: Elysia) => app.get("/api/antigravity/usage", async ({ query, set }) => {
  try { const credential = credentialService.findById(query.credential_id); if (!credential || credential.kind !== "antigravity") throw new Error("Antigravity credential not found"); return await antigravityUsage(credential); }
  catch (error: any) { set.status = 502; return { error: error.message }; }
}, { query: t.Object({ credential_id: t.String() }) });
