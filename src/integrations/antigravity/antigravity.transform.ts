...
function contentParts(content: any) {
  if (Array.isArray(content)) return content.flatMap((p): any[] => {
...
function encodeFunctionCallId(id: any, name: any, thoughtSignature?: any) {
  const cleanId = String(id ?? "") || `call_${crypto.randomUUID().replace(/-/g, "")}`;
...
function decodeFunctionCall(value: any, fallbackName?: any): AntigravityCall {
  ...
  if (raw.startsWith(signaturePrefix)) {
    try {
      const parsed = JSON.parse(Buffer.from(raw.slice(signaturePrefix.length), "base64url").toString("utf8"));
      return { id: String(parsed.id), name: functionName(parsed.name ?? fallbackName), ...(parsed.thoughtSignature ? { thoughtSignature: String(parsed.thoughtSignature) } : {}) };
    } catch { /* Fall back to a regular call ID. */ }
  }
  return { id: raw || `call_${crypto.randomUUID().replace(/-/g, "")}`, name: functionName(fallbackName ?? raw) };
}
function googleSchema(schema: any): any {
  if (!schema || typeof schema !== "object" || Array.isArray(schema)) return { type: "OBJECT", properties: {} };
  const allowed = new Set(["type", "format", "title", "description", "nullable", "enum", "maxItems", "minItems", "properties", "required", "minProperties", "maxProperties", "items"]);
  const result: any = {};
  for (const [key, value] of Object.entries(schema)) {
    if (!allowed.has(key) || key.startsWith("$")) continue;
    if (key === "properties" && value && typeof value === "object") result.properties = Object.fromEntries(Object.entries(value).map(([name, child]) => [name, googleSchema(child)]));
...
export function toGoogleBody(body: any, projectId: string, sessionId = crypto.randomUUID()) {
  const systemParts = (body.messages ?? []).filter((m: any) => m.role === "system" || m.role === "developer").flatMap((m: any) => contentParts(m.content));
  const contents: any[] = [];
  ...
  for (const message of body.messages ?? []) {
    ...
    if (Array.isArray(message.content)) {
      for (const part of message.content) {
        if (part?.type === "tool_use") {
          ...
          parts.push({ functionCall: { id: decoded.id, name: decoded.name, args: functionArguments(part.input) }, ...(decoded.thoughtSignature ? { thoughtSignature: decoded.thoughtSignature } : {}) });
    ...
    if (message.role === "assistant" && Array.isArray(message.tool_calls)) {
      for (const call of message.tool_calls) {
        ...
        parts.push({ functionCall: { id: decoded.id, name: decoded.name, args: functionArguments(fn.arguments) }, ...(decoded.thoughtSignature ? { thoughtSignature: decoded.thoughtSignature } : {}) });
      }
    }
    contents.push({ role: message.role === "assistant" ? "model" : "user", parts: parts.length ? parts : [{ text: "" }] });
  }
  ...
  if (body.tools?.length) request.tools = [{ functionDeclarations: body.tools.map((tool: any) => { const fn = tool.function ?? tool; return { name: functionName(fn.name), description: textValue(fn.description), parameters: googleSchema(fn.parameters ?? { type: "object", properties: {} }) }; }) }];
  const choice = body.tool_choice;
  if (choice || body.parallel_tool_calls !== undefined) request.toolConfig = { functionCallingConfig: { mode: choice === "none" ? "NONE" : choice === "required" ? "ANY" : "AUTO", ...(choice?.function?.name ? { allowedFunctionNames: [functionName(choice.function.name)] } : {}) } };
