...
export function parseManualOAuthCallback(
  ...
  if (
    ...
    url.port !== "1455" ||
    url.pathname !== callbackPaths[provider] ||
    url.username ||
    url.password
  ) {
    throw new Error(
      `Expected the ${provider === "codex" ? "Codex" : "Antigravity"} localhost callback URL`,
    );
  }
  ...
  if (!code || !state)
    throw new Error("Callback URL is missing the OAuth code or state");
  return { code, state };
}
