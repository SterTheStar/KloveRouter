...
export const codexAuthService = {
  async read(): Promise<TokenFile | null> {
    try {
      return JSON.parse(await readFile(authPath(), "utf8")) as TokenFile;
    } catch {
      return null;
    }
  },
  ...
  async status() {
    ...
    return {
      ...
      warning:
        "Unofficial OAuth integration. It relies on private Codex/ChatGPT endpoints and may stop working without notice.",
    };
  },
  ...
  async startLogin(credentialId?: string) {
    ...
    if (!credential || credential.kind !== "codex")
      throw new Error("Codex credential not found");
    const now = Date.now();
    ...
    return {
      auth_url: `${ISSUER}/oauth/authorize?${params}`,
      warning:
        "Unofficial OAuth integration. Do not share your Codex credentials or auth.json.",
    };
  },
  ...
  async completeLogin(
    ...
    const credential = login.credentialId
      ? credentialService.findById(login.credentialId)
      : null;
    if (login.credentialId && (!credential || credential.kind !== "codex"))
      throw new Error("Codex credential not found");
    pending.delete(state);
    ...
    if (login.credentialId) {
      ...
      logger.success("Codex account authenticated", {
    ...
    const payload: TokenFile = {
      OPENAI_API_KEY: data.access_token ?? null,
      tokens: {
        id_token: data.id_token,
        access_token: data.access_token,
        refresh_token: data.refresh_token,
  ...
  async refresh() {
    ...
    const next = {
      ...auth,
      OPENAI_API_KEY: data.access_token ?? auth.OPENAI_API_KEY,
      tokens: {
        ...auth.tokens,
        id_token: data.id_token ?? auth.tokens?.id_token,
        access_token: data.access_token ?? auth.tokens?.access_token,
        refresh_token: data.refresh_token ?? refreshToken,
        account_id: readAccountId(data.id_token) ?? auth.tokens?.account_id,
      },
      last_refresh: new Date().toISOString(),
    };
  ...
  accountId: async () =>
    (await codexAuthService.read())?.tokens?.account_id ?? null,
};
...
export const CODEX_CALLBACK_HTML =
  "<!doctype html><html><body><h1>Codex connected</h1><p>You can close this window and return to Klove.</p></body></html>";
